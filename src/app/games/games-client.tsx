...
import { Gamepad2, ExternalLink, LayoutGrid, List } from 'lucide-react';
                    ...
                    {GAMES.map((game) => (
                        viewMode === 'grid' ? (
                            ...
                            <a
                                ...
                                target="_blank"
                                rel="noopener noreferrer"
                                className="group block rounded-xl overflow-hidden bg-white shadow-sm hover:shadow-lg transition-all duration-300 border border-gray-100"
                            >
                                <div className="flex flex-row">
                                    <div
                                        className={`relative w-24 h-20 sm:w-48 md:w-56 sm:h-36 bg-gradient-to-br ${game.color} flex items-center justify-center flex-shrink-0`}
                                    >
                                        <span className="text-4xl sm:text-5xl select-none drop-shadow-sm">
                                            {game.emoji}
                                        </span>
                                        {game.badge && (
                                            <span className="absolute top-2 right-2 bg-white/90 backdrop-blur-sm text-[10px] font-bold text-gray-700 px-2 py-0.5 rounded-full shadow-sm">
                                                {game.badge}
                                            </span>
                                        )}
                                    </div>
                                    <div className="p-2.5 sm:p-4 flex flex-col justify-center flex-1 min-w-0">
                                        <div className="flex items-start justify-between gap-2 sm:gap-3">
                                            <div className="min-w-0">
                                                <h2 className="font-bold text-sm sm:text-base text-gray-900 group-hover:text-pink-600 transition-colors truncate mb-0.5 sm:mb-1">
                                                    {game.name}
                                                </h2>
                                                <p className="text-xs sm:text-sm text-gray-500 line-clamp-1 sm:line-clamp-2 mb-1 sm:mb-2">
                                                    {game.description}
                                                </p>
                                                <span className="inline-flex items-center gap-1 text-[10px] sm:text-xs font-semibold text-white bg-gradient-to-r from-pink-500 to-rose-500 px-2.5 sm:px-3 py-1 sm:py-1.5 rounded-full">
                                                    Jugar ahora
