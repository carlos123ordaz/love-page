...
const VALUES = [
    { icon: Heart, title: 'Amor ante todo', desc: 'Cada función que creamos tiene un propósito emocional. Queremos que las personas se sientan amadas.', color: 'var(--accent-hex)' },
    { icon: Sparkles, title: 'Creatividad sin límites', desc: 'Te damos las herramientas para que tu imaginación vuele: temas, animaciones, IA, música y más.', color: 'var(--ink-black)' },
    { icon: Users, title: 'Accesible para todos', desc: 'El plan gratuito siempre existirá. Creemos que el amor no debe tener barreras económicas.', color: 'var(--accent-2-hex)' },
    { icon: Shield, title: 'Privacidad y confianza', desc: 'Tus páginas son tuyas. No vendemos tus datos ni compartimos tu contenido sin tu permiso.', color: 'var(--accent-hex)' },
    { icon: Globe, title: 'Para el mundo hispanohablante', desc: 'Somos una plataforma creada pensando en Latinoamérica y España, aunque damos la bienvenida a todos.', color: 'var(--ink-black)' },
    { icon: Star, title: 'Mejora continua', desc: 'Escuchamos a nuestra comunidad constantemente y lanzamos nuevas funciones cada semana.', color: 'var(--accent-2-hex)' },
];
    ...
    return (
        <div style={{ minHeight: '100vh', background: 'var(--paper)', color: 'var(--ink-black)', fontFamily: 'var(--mono)' }}>
            ...
            <main style={{ maxWidth: 860, margin: '0 auto' }} className="px-5 py-12 sm:px-12 sm:py-16">
                ...
                {/* Our Story */}
                <section style={{ borderTop: '1px solid var(--hairline)', borderBottom: '1px solid var(--hairline)', padding: '32px 0', marginBottom: 0, position: 'relative' }}>
                    ...
                    <svg style={{ position: 'absolute', right: 20, top: 20, width: 90, height: 90, mixBlendMode: 'multiply', opacity: 0.4, pointerEvents: 'none' }} viewBox="0 0 90 90">
                        <circle cx="45" cy="45" r="42" fill="var(--ink-black)" />
                    </svg>
                    <div className="mono-eyebrow" style={{ marginBottom: 20, color: 'var(--ink-black)' }}>nuestra historia</div>
                    <div style={{ display: 'flex', flexDirection: 'column', gap: 16, fontSize: 15, lineHeight: 1.85, color: 'var(--ink-soft)', maxWidth: 640, position: 'relative', zIndex: 1 }}>
                        ...
                        <p style={{ margin: 0 }}>Empezamos con una función que se volvió viral rápidamente: el <strong style={{ color: 'var(--ink-black)', fontFamily: 'var(--sans)', textTransform: 'none', letterSpacing: 0, fontSize: 14 }}>botón "No" que escapa</strong> cuando intentan presionarlo. Era divertido, era romántico, y la gente lo amaba.</p>
                ...
                {/* Mission */}
                <section style={{ border: '1px solid var(--hairline)', borderTop: 'none', background: 'var(--accent-hex)', color: 'var(--paper)', padding: '32px 36px', marginBottom: 0, position: 'relative', overflow: 'hidden' }}>
                    <svg style={{ position: 'absolute', right: -20, top: -20, width: 140, height: 140, mixBlendMode: 'multiply', opacity: 0.4 }} viewBox="0 0 140 140">
                        <circle cx="70" cy="70" r="65" fill="var(--ink-black)" />
                    </svg>
                    <div className="mono-eyebrow" style={{ color: 'rgba(248,241,222,0.7)', marginBottom: 16 }}>nuestra misión</div>
                    <p style={{ fontFamily: 'var(--serif)', fontSize: 'clamp(16px, 2.5vw, 22px)', lineHeight: 1.5, margin: 0, position: 'relative', zIndex: 1 }}>
                ...
                <section style={{ marginTop: 48, marginBottom: 0 }}>
                    ...
                    <div style={{ border: '1px solid var(--hairline)' }}>
                        {VALUES.map((item, i) => {
                            ...
                            return (
                                <div key={item.title} style={{ display: 'flex', alignItems: 'flex-start', gap: 20, padding: '20px 24px', borderBottom: i < VALUES.length - 1 ? '1px solid var(--hairline)' : 'none', background: i % 2 === 0 ? 'var(--paper-soft)' : 'var(--paper)' }}>
                                    ...
                                    <div>
                                        <div style={{ fontFamily: 'var(--sans)', fontSize: 15, fontWeight: 700, textTransform: 'none', letterSpacing: 0, marginBottom: 6, color: 'var(--ink-black)' }}>{item.title}</div>
                                        <div style={{ fontSize: 15, color: 'var(--ink-soft)', lineHeight: 1.7 }}>{item.desc}</div>
                                    </div>
                                </div>
                            );
                        })}
                ...
                {/* What we offer */}
                <section style={{ border: '1px solid var(--hairline)', borderTop: 'none', padding: '28px 32px', background: 'var(--paper-soft)', marginBottom: 0 }}>
                    ...
                    <ul style={{ listStyle: 'none', padding: 0, margin: 0, display: 'flex', flexDirection: 'column', gap: 10 }}>
                        {[
                            ...
                            <li key={i} style={{ display: 'flex', gap: 12, fontSize: 15, color: 'var(--ink-soft)', alignItems: 'baseline' }}>
                                <span style={{ color: i % 2 === 0 ? 'var(--accent-hex)' : 'var(--ink-black)', flexShrink: 0, fontWeight: 700 }}>→</span>
                                {item}
                            </li>
                        ))}
                    </ul>
                ...
                {/* Contact CTA */}
                <section style={{ border: '1px solid var(--hairline)', borderTop: 'none', padding: '32px', textAlign: 'center', background: 'var(--paper)', marginTop: 48 }}>
                    ...
                    <p style={{ fontSize: 15, color: 'var(--ink-soft)', marginBottom: 24 }}>Estamos aquí para ayudarte. Escríbenos y te respondemos a la brevedad.</p>
                    <div style={{ display: 'flex', gap: 12, justifyContent: 'center', flexWrap: 'wrap' }}>
                        <Link href="/contact">
                            <button className="btn-accent" style={{ padding: '12px 24px', fontSize: 15 }}>contactar →</button>
                        </Link>
                        <Link href="/create">
                            <button className="btn-ink" style={{ padding: '12px 24px', fontSize: 15 }}>crear mi página gratis →</button>
                        </Link>
                    </div>
                </section>
